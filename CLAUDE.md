...
Unlike the blog repo, there's no centralized env module here — `payload.config.ts` reading `process.env.DATABASE_URL` / `process.env.PAYLOAD_SECRET` / `process.env.CLOUDINARY_CLOUD_NAME` / `process.env.CLOUDINARY_API_KEY` / `process.env.CLOUDINARY_API_SECRET` directly is Payload's own idiomatic bootstrap pattern, not a violation to fix. Keep env reads confined to `payload.config.ts` and don't scatter `process.env` access into collection/hook/access files — pass values down through config instead. (The `CLOUDINARY_*` trio is passed to the `cloudinaryStorage` plugin as-is, i.e. `undefined` when unset, because the plugin treats "all three present" as its on/off switch — see `src/storage/`.)
